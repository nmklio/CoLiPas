...
| 智能刷新调度 | 标准模式每 15 秒、性能模式每 30 秒同步总览；标签页隐藏或浏览器离线时停止轮询，并通过 BroadcastChannel/localStorage 选出一个主同步标签页，其他标签页接收快照而不重复轮询；操作员菜单会显示同步健康、快照数和减少轮询次数，返回前台后按数据新鲜度恢复，连续失败时最长退避到 120 秒。 |
