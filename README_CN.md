...
| 浏览器 SSH | xterm 风格交互终端、WebSocket 实时输出与兼容流降级、窗口关闭清理后端会话、`Ctrl+C` 中断、可见断线恢复、不会重放命令的手动重连、复制和清屏工具。 |
...
| 移动端快捷控制 | 手机端首行保留导航、上线检查和命令面板；性能模式、语言、刷新、账户设置和退出登录集中到可访问的紧凑抽屉，不再挤占工作区。 |
