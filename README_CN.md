...
| 资源告警策略 | 在 SQLite 中持久化 CPU、内存、磁盘阈值和 15 分钟至 24 小时的再次提醒周期；只评估已验证、运行中且遥测新鲜的 SSH 资产，过期或不可用样本会明确标记并跳过，不再生成模拟告警。 |
| 可信资源历史 | 仅在 SSH 指标采集成功后记录 CPU、内存和磁盘，区分真实与模拟来源，并提供 1 小时、6 小时、24 小时和 7 天趋势、最新值、平均值、峰值、变化与连续性；仅登记资产和失败样本不会入库，响应脱敏且最多返回 240 点。 |
| 全局运维收件箱 | 汇总资源越线、上线阻塞、SSH 与资产覆盖缺口和开放事件，按优先级分组并直接跳转处理模块；本机审阅状态只保存安全的稳定 ID 与时间，持续越线会按策略周期再次提醒。 |
| 自适应操作员控制 | 桌面端和手机端始终显示登录名称与同步状态，把刷新、语言、账户设置和退出登录集中到可访问的溢出面板；窄屏仍保留运维收件箱、上线检查和命令面板，不再形成多行顶栏。 |
| 智能刷新调度 | 标准模式每 15 秒、性能模式每 30 秒同步总览；标签页隐藏或浏览器离线时停止轮询，通过 BroadcastChannel/localStorage 选出一个主同步标签页，并在资产未变化时用私有 ETag 快照和 `304` 响应复用数据。操作员菜单会显示快照数、减少轮询、载荷复用次数和节省字节，返回前台后按数据新鲜度恢复，连续失败时最长退避到 120 秒。 |
| 意图就绪导航 | 当前工作区会保持可用，直到所选模块准备完成；指针、焦点与点击共享同一次准备请求，连续切换时只接受最后一次选择。标准模式仅在页面可见、网络在线且未启用省流量或 2G 网络时分阶段准备模块，性能模式只加载明确选择的工作区。 |
...
| `COLIPAS_METRIC_HISTORY_INTERVAL_MS` / `COLIPAS_METRIC_HISTORY_RETENTION_MS` / `COLIPAS_METRIC_HISTORY_MAX_POINTS_PER_SERVER` | 成功 SSH 指标的历史采样间隔、保留时长和单服务器行数上限，默认分别为 5 分钟、7 天和 2016 行。 |
