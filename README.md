...
| Adaptive refresh scheduler | Refreshes Overview every 15 seconds in standard mode or 30 seconds in Performance mode, pauses polling while the tab is hidden or the browser is offline, elects one primary sync tab with BroadcastChannel/localStorage so additional tabs receive snapshots instead of repeating the same poll, resumes from data freshness on return, and applies failure backoff up to 120 seconds. |
