...
6. Set CPU, memory, and disk alert thresholds on Overview, then triage recurring resource breaches alongside release blockers, SSH coverage gaps, and open events in the global Operations inbox.
...
| Resource alert policy | Persists CPU, memory, and disk thresholds plus a 15-minute to 24-hour re-alert cadence in SQLite. Verified running SSH assets are evaluated locally; breaches enter the Operations inbox as P1 and escalate to P0 ten percentage points above threshold. |
...
| Intent-ready navigation | Keeps the current workspace usable until the selected module is ready, merges pointer/focus and click preparation into one request, and lets the newest selection win when navigation overlaps. Standard mode prepares modules in stages only while visible and online, with Save-Data and 2G safeguards; Performance mode loads only explicitly selected workspaces. |
