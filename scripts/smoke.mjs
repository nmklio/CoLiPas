...
const runbookSecondResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook`, {
  method: 'POST',
  headers: { ...authHeaders, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    title: `${runbookSmokeTitle} second`,
    command: 'printf colipas-runbook-second',
  }),
});
if (runbookSecondResponse.status !== 201) {
  throw new Error(`/api/servers/ssh-runbook expected 201 on second create, got ${runbookSecondResponse.status}: ${await runbookSecondResponse.text()}`);
}
const runbookSecond = await runbookSecondResponse.json();
const runbookReorderResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/reorder`, {
  method: 'POST',
  headers: { ...authHeaders, 'Content-Type': 'application/json' },
  body: JSON.stringify({ commandIds: [runbookUpdated.id, runbookSecond.id] }),
});
if (!runbookReorderResponse.ok) {
  throw new Error(`/api/servers/ssh-runbook/reorder returned HTTP ${runbookReorderResponse.status}: ${await runbookReorderResponse.text()}`);
}
const runbookReordered = await runbookReorderResponse.json();
if (runbookReordered.commands?.[0]?.id !== runbookUpdated.id || runbookReordered.commands?.[1]?.id !== runbookSecond.id) {
  throw new Error(`/api/servers/ssh-runbook/reorder did not persist order: ${JSON.stringify(runbookReordered)}`);
}

const runbookPinResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/${encodeURIComponent(runbookSecond.id)}/pin`, {
  method: 'PATCH',
  headers: { ...authHeaders, 'Content-Type': 'application/json' },
  body: JSON.stringify({ pinned: true }),
});
if (!runbookPinResponse.ok) {
  throw new Error(`/api/servers/ssh-runbook/:id/pin returned HTTP ${runbookPinResponse.status}: ${await runbookPinResponse.text()}`);
}
const runbookPinned = await runbookPinResponse.json();
if (runbookPinned.command?.id !== runbookSecond.id || runbookPinned.command?.pinned !== true || runbookPinned.commands?.[0]?.id !== runbookSecond.id) {
  throw new Error(`/api/servers/ssh-runbook/:id/pin did not pin to the front: ${JSON.stringify(runbookPinned)}`);
}
const runbookUnpinResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/${encodeURIComponent(runbookSecond.id)}/pin`, {
  method: 'PATCH',
  headers: { ...authHeaders, 'Content-Type': 'application/json' },
  body: JSON.stringify({ pinned: false }),
});
if (!runbookUnpinResponse.ok) {
  throw new Error(`/api/servers/ssh-runbook/:id/pin unpin returned HTTP ${runbookUnpinResponse.status}: ${await runbookUnpinResponse.text()}`);
}
const runbookUnpinned = await runbookUnpinResponse.json();
...
const runbookSecondDeleteResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/${encodeURIComponent(runbookSecond.id)}`, {
  method: 'DELETE',
  headers: authHeaders,
});
if (!runbookSecondDeleteResponse.ok) {
  throw new Error(`/api/servers/ssh-runbook/:id second delete returned HTTP ${runbookSecondDeleteResponse.status}`);
}
const runbookImportResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/import`, {
  method: 'POST',
  headers: { ...authHeaders, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    commands: [
      { title: `${runbookSmokeTitle} pack system`, command: 'printf colipas-runbook-pack-system' },
      { title: `${runbookSmokeTitle} pack logs`, command: 'journalctl -p warning -n 5 --no-pager' },
      { title: `${runbookSmokeTitle} pack system`, command: 'printf colipas-runbook-pack-duplicate' },
    ],
  }),
});
if (runbookImportResponse.status !== 201) {
  throw new Error(`/api/servers/ssh-runbook/import expected 201, got ${runbookImportResponse.status}: ${await runbookImportResponse.text()}`);
}
const runbookImported = await runbookImportResponse.json();
if (runbookImported.imported?.length !== 2 || runbookImported.skipped?.length !== 1 || runbookImported.commands?.[0]?.title !== `${runbookSmokeTitle} pack system`) {
  throw new Error(`/api/servers/ssh-runbook/import returned unexpected payload: ${JSON.stringify(runbookImported)}`);
}
for (const importedCommand of runbookImported.imported) {
  const cleanupResponse = await fetch(`${baseUrl}/api/servers/ssh-runbook/${encodeURIComponent(importedCommand.id)}`, {
    method: 'DELETE',
    headers: authHeaders,
  });
  if (!cleanupResponse.ok) {
    throw new Error(`/api/servers/ssh-runbook/:id imported cleanup returned HTTP ${cleanupResponse.status}`);
  }
}
...
if (
  ...
  || shellSelfTestBody.firstResponseMs !== 9
  || shellSelfTestBody.outputSpanMs !== 39
...
if (
  ...
  || diagnosticExportBody.sshTerminal?.lastSelfTest?.firstResponseMs !== 9
  || diagnosticExportBody.sshTerminal?.lastSelfTest?.outputSpanMs !== 39
...
function assertAccountUiGuards() {
  ...
  const releaseCockpitFragments = [
    ...
    'data-release-fix-router="true"',
    'data-release-fix-step={step.module}',
    'buildReleaseFixRouter',
    'releaseFixSanitizedNote',
    'security-release-fix-route',
    'data-release-fix-checklist="true"',
    'data-release-fix-checklist-item={item.id}',
    'data-release-fix-checklist-status={item.status}',
    'releaseFixChecklistStorageKey',
    'loadReleaseFixChecklistState',
    'saveReleaseFixChecklistState',
    'buildReleaseFixChecklist',
    'copyReleaseFixChecklist',
    'releaseFixChecklistSanitizedNote',
    'security-release-fix-checklist-progress',
  ...
  const releaseFixFocusFragments = [
    ...
    'data-release-focus-anchor=',
    'activeReleaseFixFocus',
    'scrollReleaseFocusAnchor',
...
function assertSqlitePersistenceGuards() {
  ...
  const sshTerminalQualityFragments = [
    ...
    'getTerminalQualityInsight',
    'servers.terminalQualityLatencyTitle',
    'servers.terminalQualityCompatibleDetail',
    'TerminalTelemetryState',
    'getTerminalTelemetryInsight',
    'recordTerminalInput',
    'recordTerminalOutput',
    'terminalRenderForceLagMs',
    'terminalRenderForceBacklogThreshold',
    ...
    '.ssh-terminal-support-bundle',
    'TerminalPasteReview',
    'data-ssh-paste-review="true"',
    'shouldReviewTerminalPaste',
    'buildTerminalPasteReview',
    ...
    '.ssh-terminal-refresh-mode',
    '.ssh-paste-review',
    '.ssh-terminal-telemetry-grid',
    '.ssh-terminal-screen {\n  order: 3;',
    '.ssh-terminal-recovery {\n  order: 2;',
    'flex: 1 0 clamp(320px, 52vh, 520px)',
    'overflow-y: auto',
    '.ssh-terminal-titlebar {\n  order: 1;',
    '.ssh-quick-command-deck {\n  order: 4;',
    'ssh-terminal-quality',
    '.ssh-terminal-quality.good',
    '.ssh-terminal-quality.warn',
    '.ssh-terminal-quality.slow',
  ...
  const sshBottleneckTrendFragments = [
    ...
    'copySshBottleneckTrendReport',
    'data-ssh-bottleneck-trend="true"',
    'data-ssh-bottleneck-trend-lane',
    'bottleneckTrendTitle',
    'bottleneckTrendCopy',
    'bottleneckTrendReportTitle',
    'bottleneckTrendReportPrimary',
  ...
  const sshSupportBundleFragments = [
    ...
    'copySshTerminalSnapshot',
    'copySshTerminalSnapshotInbox',
    ...
    'supportBundleSectionTerminal',
    'terminalSnapshotTitle',
    'terminalSnapshotEmpty',
    'terminalSnapshotCopy',
    'terminalSnapshotCopied',
    'terminalSnapshotCopyEmpty',
    'terminalSnapshotInboxTitle',
    'terminalSnapshotInboxDetail',
    'terminalSnapshotInboxCopy',
    'terminalSnapshotInboxCopied',
    'sshTerminalSupportSnapshotStorageKey',
    'sshTerminalSupportSnapshotHistoryStorageKey',
    'sshTerminalSupportSnapshotEventName',
    'loadSshTerminalSupportSnapshot',
    'loadSshTerminalSupportSnapshotHistory',
    'normalizeSshTerminalSupportSnapshot',
    'normalizeSshTerminalSupportSnapshotHistory',
    'mapTerminalSnapshotTone',
    'data-ssh-terminal-support-snapshot="true"',
    ...
    '__colipasCopiedSshTerminalSnapshotText',
    '__colipasCopiedSshTerminalInboxText',
    '.security-ssh-support-bundle',
    '.security-ssh-support-bundle-actions',
    '.security-ssh-support-bundle-sections',
    '.security-ssh-terminal-snapshot',
    '.security-ssh-terminal-snapshot-head',
    '.security-ssh-terminal-snapshot-grid',
    '.security-ssh-terminal-inbox',
    'data-ssh-terminal-support-inbox="true"',
    'data-ssh-terminal-inbox-copy="true"',
    'data-ssh-terminal-snapshot-copy="true"',
    ...
    'Security terminal snapshot copy output leaked a raw IP address or secret',
    'Security terminal evidence inbox copy output leaked a raw IP address or secret',
...
function assertOverviewMapInteractionGuards() {
  ...
  const requiredFragments = [
    ...
    'function calculateResourceAverages',
    ...
    'data-overview-preflight-command="true"',
    'data-overview-preflight-snapshot="true"',
    'overview.opsPreflightCommandKicker',
    'overview.opsPreflightGenerated',
    'onClick={() => onOpsPreflightTraceOpen?.(opsPreflightSnapshot.correlationId)}',
  ...
  assertFileContains('src/modules/operations/OperationsCenter.tsx', [
    ...
    'data-ops-draft-locator-actions="true"',
    'data-ops-draft-locator-action={action.target}',
    'data-ops-preflight-remediation="true"',
    'data-ops-preflight-remediation-action="true"',
  ...
  const cssFragments = [
    ...
    '.nezha-map.is-zoomed',
    '.map-tooltip-action',
    '.overview-preflight-command',
    '.overview-preflight-command-actions',
    '.health-preflight-snapshot',
    'pointer-events: none',
    '.map-tooltip.pinned',
    'pointer-events: auto',
    '.region-row:hover',
...
function assertSshTerminalRealtimeGuards() {
  ...
  const requiredFrontendFragments = [
    ...
    'function recoverTerminalSession(',
    ...
    'shouldReviewTerminalPaste(data)',
    'openTerminalPasteReview(sessionId, data)',
    'function sendReviewedTerminalPaste()',
    ...
    'data-ssh-paste-review-cancel="true"',
    ...
    'TerminalTelemetryState',
    'terminalTelemetryRef',
    'function updateTerminalTelemetry(',
    'terminalTelemetryUiRefreshMs = 900',
    'terminalNetworkUiRefreshMs = 1500',
    'renderLagMs >= terminalRenderForceLagMs || pendingBytes >= terminalRenderForceBacklogThreshold',
    'terminalFocusModeStorageKey',
    'terminalLiteModeStorageKey',
    'terminalWorkspaceModeStorageKey',
    'terminalLatencyReportStorageKey',
    'terminalLatencyReportHistoryStorageKey',
    'terminalFocusModeNetworkUiRefreshMs',
    'terminalFocusModeTelemetryUiRefreshMs',
    'terminalDiagnosticsExpandedStorageKey',
    'const [terminalFocusMode, setTerminalFocusMode]',
    'const [terminalLiteMode, setTerminalLiteMode]',
    'const [terminalDiagnosticsExpanded, setTerminalDiagnosticsExpanded]',
    'const [terminalWorkspaceMode, setTerminalWorkspaceMode]',
    ...
    'const terminalWorkspaceLayouts',
    'function deriveTerminalWorkspaceMode(',
    'function readTerminalWorkspaceMode()',
    ...
    "className={terminalFocusMode ? 'ssh-terminal-shell focus-mode' : 'ssh-terminal-shell'}",
    'data-ssh-terminal-workspace-picker="true"',
    'data-ssh-terminal-workspace-mode="true"',
    'data-ssh-terminal-tools="true"',
    'data-ssh-terminal-focus-toggle="true"',
    'data-ssh-terminal-lite-toggle="true"',
    'data-ssh-terminal-lite-summary="true"',
    ...
    'data-ssh-diagnostics-chip={item.id}',
    'terminalDiagnosticsExpanded && terminalLatencyReport',
    'terminalDiagnosticsExpanded && terminalSupportBundle',
    ...
    'data-ssh-terminal-latency-report-section={section.id}',
    'data-ssh-latency-report-copy="true"',
    'function copyTerminalLatencyReport()',
    'function persistTerminalLatencyReportSnapshot(',
    'TerminalSupportBundle',
    'function buildTerminalSupportBundle(',
    'const terminalSupportBundle = useMemo',
    'data-ssh-terminal-support-bundle="true"',
    'data-ssh-terminal-support-section={section.id}',
    'function copyTerminalSupportBundle()',
    'function persistTerminalSupportBundleSnapshot(',
    'sshTerminalSupportSnapshotStorageKey',
    'sshTerminalSupportSnapshotHistoryStorageKey',
    ...
    'normalizeSshTerminalSupportSnapshot',
    'const [terminalChannelSwitching, setTerminalChannelSwitching]',
    'data-ssh-channel-switch="true"',
    'function switchTerminalChannel()',
    ...
    'const [importingSshRunbookPackId, setImportingSshRunbookPackId]',
    'const [pinningSshRunbookId, setPinningSshRunbookId]',
    'const [sshRunbookSearch, setSshRunbookSearch]',
    'const [sshRunbookCategory, setSshRunbookCategory]',
    ...
    'function countSshRunbookCategories(',
    'function saveSshRunbookCommand(',
    'function startEditSshRunbookCommand(',
    ...
    'data-ssh-runbook-filter-empty="true"',
  ...
  const requiredToolLabels = [
    ...
    'servers.terminalWorkspaceLabel',
    'servers.terminalWorkspaceOperate',
    'servers.terminalWorkspaceFocus',
    'servers.terminalWorkspaceDiagnose',
    'servers.terminalWorkspaceCustom',
    'servers.terminalRefreshModeLabel',
    'servers.terminalRefreshModeDetail',
    'servers.terminalFocusOn',
    'servers.terminalFocusOff',
    'servers.terminalLiteModeOn',
    'servers.terminalLiteModeOff',
    'servers.terminalLiteModeEyebrow',
    'servers.terminalLiteModeExpand',
    ...
    'servers.sshPasteReviewEyebrow',
    'servers.sshPasteReviewTitle',
    'servers.sshPasteReviewDetail',
    'servers.sshPasteReviewQueued',
    'servers.sshPasteReviewSend',
    'servers.sshPasteReviewCancel',
    'servers.sshPasteReviewSent',
    'servers.terminalTransportWebSocket',
    'servers.terminalTransportCompatible',
    'servers.terminalTransportPending',
    'servers.terminalQualityConnectingTitle',
    'servers.terminalQualityPendingTitle',
    'servers.terminalQualityCompatibleTitle',
    'servers.terminalQualityGoodTitle',
    'servers.terminalQualityLatencyTitle',
    'servers.terminalQualityThroughputTitle',
    'servers.terminalQualitySelfTestTitle',
    'servers.telemetryTitle',
    'servers.telemetryInputLabel',
    ...
    'servers.terminalLagActionNetworkTitle',
    'servers.terminalLagActionOutputTitle',
    'servers.terminalLagActionRenderTitle',
    'servers.terminalLagActionInputTitle',
    'servers.terminalLagActionSelfTestButton',
    'servers.terminalLagActionClearButton',
    ...
    'servers.terminalLatencyReportCopied',
    'servers.terminalLatencyReportSanitized',
    'servers.terminalLatencyReportInputEcho',
    'servers.terminalLatencyReportFirstOutput',
    'servers.terminalLatencyReportThroughput',
    'servers.terminalLatencyReportRender',
    'servers.terminalLatencyReportChannel',
    'servers.terminalLatencyReportAction',
    'servers.switchToCompatibleChannel',
    'servers.retryWebSocketChannel',
    'servers.switchingToCompatibleChannel',
    'servers.retryingWebSocketChannel',
    'servers.switchedToCompatibleChannel',
    'servers.retriedWebSocketChannel',
    ...
    'servers.quickCommandUpdate',
    'servers.quickCommandCancelEdit',
    'servers.quickCommandSearchPlaceholder',
    'servers.quickCommandFilterLabel',
    'servers.quickCommandFilterClear',
    ...
    'servers.quickCommandCategory.',
    'servers.quickCommandEdit',
    'servers.quickCommandEditing',
    'servers.quickCommandUpdated',
    'servers.quickCommandMoved',
    'servers.quickCommandReorderFailed',
    'servers.quickCommandPinned',
    ...
    'servers.quickCommandEditAria',
    'servers.quickCommandMoveUpAria',
    'servers.quickCommandMoveDownAria',
  ...
  const terminalLatencyReportI18nKeys = [
    'servers.terminalLatencyReportEyebrow',
    'servers.terminalLatencyReportTitle',
    'servers.terminalLatencyReportDetail',
    'servers.terminalLatencyReportCopy',
    'servers.terminalLatencyReportCopied',
    'servers.terminalLatencyReportGenerated',
    'servers.terminalLatencyReportSanitized',
    'servers.terminalLatencyReportSafeNote',
    'servers.terminalLatencyReportInputEcho',
    'servers.terminalLatencyReportInputEchoPending',
    'servers.terminalLatencyReportInputEchoDetail',
    'servers.terminalLatencyReportFirstOutput',
    'servers.terminalLatencyReportFirstOutputPending',
    'servers.terminalLatencyReportFirstOutputDetail',
    'servers.terminalLatencyReportSourceLive',
    'servers.terminalLatencyReportSourceSelfTest',
    'servers.terminalLatencyReportSourcePending',
    'servers.terminalLatencyReportThroughput',
    'servers.terminalLatencyReportThroughputDetail',
    'servers.terminalLatencyReportRender',
    'servers.terminalLatencyReportRenderDetail',
    'servers.terminalLatencyReportChannel',
    'servers.terminalLatencyReportChannelDetail',
    'servers.terminalLatencyReportAction',
    'servers.terminalLatencyReportActionDetail',
  ];
  ...
  const runbookLensI18n = [
    'servers.quickCommandSearchPlaceholder',
    'servers.quickCommandFilterLabel',
    'servers.quickCommandFilterClear',
    'servers.quickCommandFilterEmptyTitle',
    'servers.quickCommandFilterEmptyDetail',
    'servers.quickCommandPackEyebrow',
    'servers.quickCommandPackTitle',
    'servers.quickCommandPackDetail',
    'servers.quickCommandPackCommandCount',
    'servers.quickCommandPackImport',
    'servers.quickCommandPackImported',
    'servers.quickCommandPackImportFailed',
    'servers.quickCommandPinned',
    'servers.quickCommandUnpinned',
    'servers.quickCommandPinFailed',
    'servers.quickCommandPinnedLabel',
    ...
    'servers.quickCommandViewLabel',
    'servers.quickCommandView.manual',
    'servers.quickCommandView.recent',
    ...
    'servers.quickCommandPack.system.processes.title',
    'servers.quickCommandPack.system.services.title',
    'servers.quickCommandPack.docker.title',
    'servers.quickCommandPack.docker.detail',
    'servers.quickCommandPack.docker.containers.title',
    'servers.quickCommandPack.docker.disk.title',
    'servers.quickCommandPack.docker.logs.title',
    'servers.quickCommandPack.network.title',
    'servers.quickCommandPack.network.detail',
    'servers.quickCommandPack.network.addresses.title',
    'servers.quickCommandPack.network.ports.title',
    'servers.quickCommandPack.network.dns.title',
    'servers.quickCommandPack.security.title',
    'servers.quickCommandPack.security.detail',
    'servers.quickCommandPack.security.auth.title',
    'servers.quickCommandPack.security.sessions.title',
    'servers.quickCommandPack.security.firewall.title',
    'servers.quickCommandCategory.all',
    'servers.quickCommandCategory.system',
    'servers.quickCommandCategory.network',
    'servers.quickCommandCategory.storage',
    'servers.quickCommandCategory.logs',
    'servers.quickCommandCategory.other',
  ];
  ...
  if (!globalCssSource.includes('.ssh-terminal-experience-center') || !globalCssSource.includes('.ssh-terminal-experience-score') || !globalCssSource.includes('.ssh-terminal-experience-pills') || !globalCssSource.includes('.ssh-terminal-diagnostics-summary') || !globalCssSource.includes('.ssh-terminal-diagnostics-chips') || !globalCssSource.includes('.ssh-terminal-diagnostics-summary > button') || !globalCssSource.includes('.ssh-terminal-latency-report') || !globalCssSource.includes('.ssh-terminal-latency-report-grid') || !globalCssSource.includes('.ssh-terminal-latency-report-copy') || !globalCssSource.includes('.ssh-quick-command-deck') || !globalCssSource.includes('.ssh-runbook-workspace') || !globalCssSource.includes('.ssh-runbook-form') || !globalCssSource.includes('.ssh-runbook-form.editing') || !globalCssSource.includes('.ssh-runbook-lens') || !globalCssSource.includes('.ssh-runbook-categories button.active') || !globalCssSource.includes('.ssh-runbook-views button.active') || !globalCssSource.includes('.ssh-runbook-view-hint') || !globalCssSource.includes('.ssh-runbook-clear-filter') || !globalCssSource.includes('.ssh-runbook-recommendations') || !globalCssSource.includes('.ssh-runbook-pack-dock') || !globalCssSource.includes('.ssh-runbook-pack-grid') || !globalCssSource.includes('.ssh-quick-command-grid') || !globalCssSource.includes('.ssh-quick-command-grid article.custom.pinned') || !globalCssSource.includes('.ssh-quick-command-grid .ssh-runbook-usage') || !globalCssSource.includes('.ssh-quick-command-grid button.pin') || !globalCssSource.includes('.ssh-quick-command-grid button.sort') || !globalCssSource.includes('.ssh-terminal-refresh-mode') || !globalCssSource.includes('.ssh-terminal-primary-tools') || !globalCssSource.includes('.ssh-terminal-workspace-picker') || !globalCssSource.includes('.ssh-terminal-tools-panel') || !globalCssSource.includes('.ssh-terminal-interrupt') || !globalCssSource.includes('.ssh-terminal-shell.focus-mode .ssh-terminal-screen') || !globalCssSource.includes('.ssh-terminal-root-cause') || !globalCssSource.includes('.ssh-terminal-root-cause-meter') || !globalCssSource.includes('.ssh-terminal-root-cause-lanes') || !globalCssSource.includes('flex-direction: column') || !globalCssSource.includes('overflow-y: auto') || !globalCssSource.includes('order: 2') || !globalCssSource.includes('flex: 1 0 clamp(320px, 52vh, 520px)') || !globalCssSource.includes('min-height: clamp(300px, 46vh, 500px)')) {
...
function assertSshConnectionDoctorGuards() {
  ...
  const requiredInventoryFragments = [
    ...
    'data-ssh-channel-check-run="true"',
    'data-ssh-channel-check="true"',
    'data-ssh-channel-check-stage={stage.id}',
    'data-ssh-channel-fix-plan="true"',
    'data-ssh-channel-fix-plan-copy="true"',
    'data-ssh-channel-fix-plan-action={action.id}',
    'copySshDoctorSummary',
    'copySshTroubleshootingReport',
    'copySshChannelFixPlan',
  ...
  const requiredI18nKeys = [
    ...
    'servers.sshChannelCheckRun',
    'servers.sshChannelCheckRunning',
    'servers.sshChannelCheckEyebrow',
    'servers.sshChannelCheckWebSocket',
    'servers.sshChannelCheckCompatible',
    'servers.sshChannelCheckCleanup',
    'servers.sshChannelCheckComplete',
    'servers.sshChannelCheckIssueFound',
    'servers.sshChannelFixPlanTitle',
    'servers.sshChannelFixPlanCopy',
    'servers.sshChannelFixPlanCopied',
    'servers.sshChannelFixWebSocketTitle',
    'servers.sshChannelFixCompatibleTitle',
    'servers.sshChannelFixCleanupTitle',
  ];
  ...
  const requiredCssFragments = [
    ...
    '.ssh-channel-check',
    '.ssh-channel-check-stages',
    '.ssh-channel-fix-plan',
    '.ssh-channel-fix-plan-actions',
...
function assertSecurityAuditRelationsAreSpecific() {
  ...
  const requiredFragments = [
    ...
    'data-security-ops-preflight-brief="true"',
    'data-security-ops-preflight-trace="true"',
    'data-security-ops-preflight-open="true"',
    'security.opsPreflightTitle',
    ...
    'data-release-evidence-timeline="true"',
    'ReleaseEvidenceTimelineSummary',
    'function buildReleaseEvidenceTimeline(',
    'function focusReleaseTimelineTrace(correlationId: string)',
    'async function copyReleaseTimeline()',
    'data-release-timeline-item={item.id}',
    'data-release-timeline-operations="true"',
    'releaseTimelineTitle',
    '发布修复清单',
    'リリース修復チェックリスト',
    ...
    'className="security-audit-main"',
    'className="security-audit-meta"',
    ...
    'SshExperienceSummary',
    'SshExperienceScoreSummary',
    'clampSshExperienceScore',
    'mapSshScoreTone',
    'sshPerformance.score.segments.map',
    'experienceTitle',
    'experienceEvidenceValue',
    'experienceActionValue',
    'experienceScoreTitle',
    'experienceScoreAction',
    'experienceScoreSegmentDetail',
    'SshExperienceSlaSnapshot',
    'SshExperienceSlaTrend',
    'sshExperienceSlaHistoryStorageKey',
    'loadSshExperienceSlaHistory',
    'saveSshExperienceSlaHistory',
    'createSshExperienceSlaSnapshot',
    'buildSshExperienceSlaTrend',
    'isSshExperienceSlaSnapshot',
    'data-ssh-experience-sla="true"',
    'data-ssh-experience-sla-sample="true"',
    'experienceSlaTitle',
    'experienceSlaTrendTitle',
    'experienceSlaBaseline',
    'experienceSlaBaselineAction',
    'experienceSlaReportBaseline',
    'experienceSlaSanitizedNote',
    'SSH experience SLA trend',
    'Current vs baseline',
    'copySshPerformanceSummary',
    'navigator.clipboard.writeText(sshPerformance.copyText)',
    'sshPerformanceCopy.copyCopied',
    'copyText',
    'summaryNextAction',
    ...
    'detailTitle',
    'expandedSshMetricId',
    'data-ssh-performance-detail="true"',
    'data-ssh-experience-summary="true"',
    'data-ssh-experience-score="true"',
    'data-ssh-experience-score-segment={segment.id}',
    'copySshLagReport',
    'reportFindings',
    'reportContext',
    'reportContextLabel',
    ...
    'reportEvidenceLevel',
    'reportText',
    'data-ssh-lag-report="true"',
    'reportSanitizedNote',
    'reportSanitizedBadge',
    'SshLagReportSnapshot',
    'SshFlightRecorderSummary',
    'sshLagReportHistoryStorageKey',
    'loadSshLagReportHistory',
    'saveSshLagReportSnapshot',
    'buildSshLagReportComparison',
    'buildSshFlightRecorder',
    'buildSshFlightRecord',
    'buildSshFlightDiagnosis',
    'buildSshFlightCopyText',
    'buildSshLatencyCurve',
    'buildSshInteractionSampler',
    'getSshFirstOutputMs',
    'getSshLatencyEventWeight',
    'getSshLatencyPointType',
    'formatSshFlightEventType',
    'copySshFlightReport',
    'copySshSamplerReport',
    'flightSanitizedNote',
    'samplerSanitizedNote',
    'samplerTitle',
    'samplerScoreLabel',
    'inferSshSnapshotTone',
    'containsSensitiveReportText',
    'data-ssh-lag-history="true"',
    'data-ssh-lag-history-compare="true"',
    'data-ssh-flight-recorder="true"',
    'data-ssh-latency-curve="true"',
    'data-ssh-interaction-sampler="true"',
    'sshPerformance.groups.map',
    'data-ssh-performance-card="true"',
    'security-ssh-performance-actions',
    'security-ssh-performance-groups',
    'security-ssh-performance-group-title',
    'security-ssh-performance-grid',
    'security-ssh-performance-metric',
    'sshPerformanceCopyByLanguage',
    'nextActionNoEvidence',
  ...
  const cssFragments = [
    ...
    '.security-release-timeline-actions',
    '.security-ops-preflight-brief',
    '.security-ops-preflight-metrics',
    '.security-ops-preflight-actions',
    '.security-ssh-performance-card',
    '.security-ssh-performance-actions',
    '.security-ssh-performance-actions > span',
    '.security-ssh-scoreboard',
    '.security-ssh-scoreboard-meter',
    '.security-ssh-scoreboard-lanes',
    '.security-ssh-sla-trend',
    '.security-ssh-sla-trend-head',
    '.security-ssh-sla-trend-actions small.warn',
    '.security-ssh-sla-trend-rail',
    '.security-ssh-sla-trend-timeline',
    '.security-ssh-sla-trend-empty',
    '.security-ssh-experience-summary',
    '.security-ssh-experience-lead',
    '.security-ssh-experience-grid',
    '.security-ssh-experience-card',
    '.security-ssh-performance-groups',
    '.security-ssh-performance-group',
    '.security-ssh-performance-group-title',
    '.security-ssh-performance-metric.selected',
    '.security-ssh-performance-detail',
    '.security-ssh-lag-report',
    '.security-ssh-lag-report-context',
    '.security-ssh-lag-history',
    '.security-ssh-lag-history-compare',
    '.security-ssh-lag-history-list',
    '.security-ssh-lag-history-empty',
    '.security-ssh-flight-recorder',
    '.security-ssh-flight-header',
    '.security-ssh-flight-actions',
    '.security-ssh-flight-diagnosis',
    '.security-ssh-flight-stats',
    '.security-ssh-flight-list',
    '.security-ssh-flight-rail',
    '.security-ssh-flight-rail span.input',
    '.security-ssh-flight-rail span.output',
    '.security-ssh-flight-rail span.error',
    '.security-ssh-latency-curve',
    '.security-ssh-latency-heading',
    '.security-ssh-latency-chart',
    '.security-ssh-latency-legend',
    '.security-ssh-latency-markers',
    '.security-ssh-sampler',
    '.security-ssh-sampler-heading',
    '.security-ssh-sampler-score',
    '.security-ssh-sampler-stats',
    '.security-ssh-sampler-steps',
    '.security-ssh-lag-report.warn',
    '.security-ssh-lag-report.fail',
    '.security-ssh-performance-grid',
    '.security-ssh-performance-metric',
    '.security-ssh-performance-card.fail',
    '.config-state.action',
  ];
  const missingCss = cssFragments.filter((fragment) => !globalCss.includes(fragment));
  if (missingCss.length) {
...
function assertMaintenanceWindowGuards() {
  ...
  const uiRequired = [
    ...
    'data-ops-maintenance-status-count="true"',
    'data-ops-maintenance-duration',
