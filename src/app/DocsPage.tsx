...
const resourceAlertPolicyUsageBlock = {
  icon: BellRing,
  title: '配置资源告警策略',
  body: '在总览的健康基线中打开资源告警抽屉，为 CPU、内存和磁盘设置独立阈值，并选择未解消越线的再次提醒周期。策略保存在 SQLite；告警只消费最新一次成功采集的可信遥测。',
  points: ['阈值范围为 50%–95%，再次提醒可选 15 分钟至 24 小时', '只评估 SSH 已验证、当前未停止且遥测新鲜的服务器', '采集失败时保留最后一次可信值并标记过期；从未成功采集则标记不可用，两者都不会触发告警', '策略读取完成前或读取失败时保持告警暂停，并提供重试入口', '点击收件箱告警会按服务器名称定位资产，不在浏览器存储地址、指标正文或凭据'],
