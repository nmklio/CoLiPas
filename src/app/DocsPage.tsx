...
const adaptiveRefreshUsageBlock = {
  icon: RefreshCw,
  title: '使用智能刷新调度',
  body: '总览同步会根据页面可见性、网络状态、性能模式和连续失败次数自动调整；同一浏览器打开多个后台页时会选出一个主同步标签页，备用标签页接收快照，不再让隐藏、离线或重复标签页持续产生后台请求。',
  points: ['标准模式每 15 秒同步，性能模式每 30 秒同步', '标签页隐藏或网络离线时暂停，返回前台或恢复网络后按数据新鲜度立即续传', '多标签页通过 BroadcastChannel/localStorage 协调主备角色，备用标签页不重复轮询', '连续失败采用指数退避，最长 120 秒，并保留手动刷新入口'],
