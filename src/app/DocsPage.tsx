...
const usageBlocks = [
  ...
  },
  {
    icon: ListChecks,
    title: '3. 使用上下文上线检查',
    body: '总览默认保留六项完整上线检查和修复队列；进入服务器、AI、运维、API 或安全工作区后会自动折叠为摘要，性能模式也会优先使用摘要。',
    points: ['摘要仍显示当前进度、整体状态和下一步修复', '可一键展开完整检查、证据和报告工具', '展开或摘要选择会保存在当前浏览器'],
  },
  {
    icon: TerminalSquare,
    title: '4. 打开 SSH 终端',
  ...
  },
  {
    icon: Workflow,
    title: '6. 执行运维编排',
    body: '对单台或选中的已接入服务器执行健康检查、脚本命令、重启、关机等任务；高影响操作可先登记维护窗口。',
    points: ['SSH 类任务会过滤未接入目标', '维护窗口可用快捷时长安排，高风险动作仍需二次确认', '预检覆盖、结果和审计记录会一起回写'],
  },
  {
    icon: ShieldCheck,
    title: '7. 处理安全审计',
...
const adaptiveRefreshUsageBlock = {
  icon: RefreshCw,
  title: '使用智能刷新调度',
  body: '总览同步会根据页面可见性、网络状态、性能模式和连续失败次数自动调整；同一浏览器打开多个后台页时会选出一个主同步标签页，备用标签页接收快照，操作员菜单会把当前角色、快照数和减少的重复轮询次数展示出来。',
  points: ['标准模式每 15 秒同步，性能模式每 30 秒同步', '标签页隐藏或网络离线时暂停，返回前台或恢复网络后按数据新鲜度立即续传', '多标签页通过 BroadcastChannel/localStorage 协调主备角色，备用标签页不重复轮询', '同步健康面板展示当前角色、同步节奏、快照数和减少轮询计数', '连续失败采用指数退避，最长 120 秒，并保留手动刷新入口'],
...
const accountSessionUsageBlock = {
  ...
  title: '管理登录会话',
  body: '登录会话以 SHA-256 令牌哈希和脱敏设备/时间信息保存到 SQLite，不保存原始 Cookie，因此服务重启后仍保持且可撤销；账户设置仅在页面可见且在线时每 15 秒自动同步，隐藏或离线会暂停，恢复连接后立即同步。',
  points: [
    ...
    '同步状态会明确显示活跃、暂停、离线或重试，避免把后台无效轮询误认为在线同步',
