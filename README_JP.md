...
| 安全な一括インポート | CSV / JSON のファイル選択または貼り付け内容を事前検証し、2 MB・500 行の上限、名前/IP 重複のスキップ、SQLite トランザクション書き込みを行います。パスワード、秘密鍵、Token、API Key、SSH 認証情報フィールドは拒否されます。 |
...
| オペレーター アカウント | ログイン、セッション保護、パスワード変更、3 言語切り替え、アバター設定。保存前に画像をデコード検証し、過去の壊れた画像は CoLiPas ブランドアイコンへ自動的に戻します。 |
