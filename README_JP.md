...
| 適応型更新スケジューラー | 標準モードは 15 秒、パフォーマンスモードは 30 秒ごとに Overview を同期します。タブ非表示またはオフライン時は停止し、BroadcastChannel/localStorage で 1 つの主同期タブを選び、他のタブはスナップショットを受け取って重複ポーリングを避けます。復帰時はデータ鮮度に応じて再開し、連続失敗時は最大 120 秒まで待機します。 |
