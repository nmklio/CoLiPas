...
| リソースアラートポリシー | CPU、メモリ、ディスクのしきい値と 15 分から 24 時間の再通知周期を SQLite に保存します。SSH 検証済みで稼働中かつテレメトリが新鮮な資産のみ評価し、期限切れまたは利用不可のサンプルは明示して除外します。 |
| 信頼できるリソース履歴 | SSH メトリクス取得に成功した時だけ CPU、メモリ、ディスクを記録し、実接続とシミュレーションの出所を区別します。1 時間、6 時間、24 時間、7 日の推移と最新値、平均、ピーク、変化、連続性を表示し、台帳専用・失敗サンプルは保存せず、応答は匿名化して最大 240 点に制限します。 |
| グローバル運用受信箱 | リソース超過、公開ブロッカー、SSH と資産のカバレッジ不足、未完了イベントを優先度別にまとめ、対応モジュールへ直接移動します。確認状態は安全な安定 ID と時刻だけをブラウザーに保存し、超過が続く場合は設定周期で再通知します。 |
...
| 適応型更新スケジューラー | 標準モードは 15 秒、パフォーマンスモードは 30 秒ごとに Overview を同期します。非表示またはオフライン時は停止し、BroadcastChannel/localStorage で主同期タブを選び、資産が変わらない場合はプライベート ETag スナップショットと `304` 応答を再利用します。オペレーターメニューにはスナップショット、削減ポーリング、ペイロード再利用回数、削減バイト数を表示し、復帰時は鮮度に応じて再開、連続失敗時は最大 120 秒まで待機します。 |
| インテント準備型ナビゲーション | 選択したモジュールの準備が完了するまで現在のワークスペースを利用でき、ポインター、フォーカス、クリックは同じ準備リクエストを共有します。切り替えが重なった場合は最後の選択だけを反映します。標準モードは表示中かつオンラインで、Save-Data や 2G でない場合のみ段階的に準備し、パフォーマンスモードは明示的に選択したワークスペースだけを読み込みます。 |
...
| `COLIPAS_METRIC_HISTORY_INTERVAL_MS` / `COLIPAS_METRIC_HISTORY_RETENTION_MS` / `COLIPAS_METRIC_HISTORY_MAX_POINTS_PER_SERVER` | 成功した SSH メトリクスの履歴記録間隔、保持期間、サーバーごとの行数上限。既定値は 5 分、7 日、2016 行です。 |
...
アカウント設定のログインセッションは、タブが表示中かつオンラインのときだけ 15 秒ごとに同期します。非表示またはオフラインでは一時停止し、再接続後にすぐ同期を再開します。
