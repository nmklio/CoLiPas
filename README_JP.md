...
| オペレーター アカウント | ログイン、アクティブ端末の確認、他セッションの個別または一括無効化、パスワード変更、3 言語切り替え、アバター設定。セッション一覧は生の IP や User-Agent を返さず、画像は保存前にデコード検証し、壊れた履歴画像は CoLiPas ブランドアイコンへ自動的に戻します。 |
...
- セッション Cookie は HTTP-only です。アカウント設定では匿名化されたアクティブ端末を確認し、他のセッションを個別または一括で無効化できます。パスワード変更時も他の全セッションを失効させます。
