...
| ブラウザー SSH | xterm 風の対話型端末、WebSocket のリアルタイム出力と互換ストリームへのフォールバック、閉じた時のバックエンドセッション破棄、`Ctrl+C`、可視化された切断復旧、コマンドを再送しない手動再接続、コピー、クリア。 |
...
| モバイル クイック操作 | モバイルの 1 行目にはナビゲーション、公開チェック、コマンドパレットを残し、パフォーマンスモード、言語、更新、アカウント設定、ログアウトはアクセス可能なコンパクトドロワーにまとめます。 |
